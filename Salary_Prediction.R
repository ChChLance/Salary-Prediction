...
# Check Correlation (If there are two variable with correlation coefficient >= 0.9, just keep the higher one)
...
# Split data into testing and training data set with ratio = 0.8
library(caTools)
set.seed(1)
...
# Check IV value (Keep variables with IV >= 0.3)
...
# We get the primary testing Accuracy for these three models:
## Logistic Testing Accuracy = 83.63%
## C5.0 Testing Accuracy = 85.26% (Best one)
## SVM  Testing Accuracy = 84.22%
...
# By K-fold validation, we know that C5.0 Decision tree has the greatest accuracy!
# Conclusion: The best model for predicting whether the salary is greater than 50k is to use the C5.0 Decision tree with variables that are relationship, age, occupation, education, hours_per_week, and capital_gain
